...
const AttendanceScreen = props => {
    ...
    try {
      if (Object.keys(subjectInfo).length !== 0 && course) {
        for (let semester in subjectInfo[course][batch]) {
          for (let week of subjectInfo[course][batch][semester][group][
            "week"
          ]) {
            let today = new Date();
            if (
        ...
        if (course && batch && semester) {
          lastSemIndex = Object.keys(subjectInfo[course][batch])[
            Object.keys(subjectInfo[course][batch]).length - 1
          ];
          let lastWeekIndex =
            subjectInfo[course][batch][lastSemIndex][group]["week"].length - 1;
          try {
            endSemDate =
              subjectInfo[course][batch][semester][group]["week"][lastWeekIndex]
                .endDate;
          } catch {}
        }
      }
  ...
  return (
    <>
      {!uid.uid ? null : uid.uid === "admin" ? (
        <Switch>
          <Route
            path={`${route}/admin-create`}
            render={routerProps => {
              return (
                <AdminGenerateTimeTable
