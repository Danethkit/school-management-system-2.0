...
export default connect(state => ({
  TTView: state.initData.TTView,
  editTT: state.initData.editTT
}))(({ dispatch, TTView, editTT, ...others }) => {
  ...
  const handleRowClick = data => {
    dispatch(
      requestTimeTableData({ course, batch, semester, group, week: data.name })
    );
    setOpen(true);
    setWeek({ ...data });
  };
    ...
    <Box className={classes.root} boxShadow={2}>
      <Table>
        <TableHead className={classes.tableHead}>
          <TableRow>
            {columns.map(c => (
              <TableCell className={classes.tableCell} variant="head" key={c}>
                {c}
              </TableCell>
            ))}
          </TableRow>
